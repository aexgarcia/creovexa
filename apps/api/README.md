...
La API incluye el catálogo de productos y plantillas y la creación, listado y detalle de campañas. Swagger está en <http://localhost:3001/docs>; OpenAPI, en <http://localhost:3001/openapi.json>. Las respuestas de detalle usan `{ data }` y los listados `{ data, meta }`, con `page` y `limit`.
...
Consulta [la guía de desarrollo](../../README.md) y [las decisiones del catálogo](../../docs/phase-4-catalog.md) y [el incremento de campañas](../../docs/phase-4-campaigns.md).
